...
public class HoaDonService : IHoaDonService
{
    ...
    public async Task<HoaDonDto> CreateHoaDonAsync(CreateHoaDonDto createDto)
    {
        try
        {
            ...
            {
                // Nếu bàn đang sử dụng nhưng không có hóa đơn chưa thanh toán, có thể là lỗi dữ liệu
                // Cập nhật lại trạng thái về "Trống" hoặc "Đã đặt"
                ban.TrangThai = "Trống";
        ...
        try
        {
            ...
            if (maKH.HasValue && thanhToanDto.DiemSuDung > 0)
            {
                ...
                if (khachHang != null)
                {
                    ...
                    if (tienGiamTuDiem > hoaDon.TongTien)
                    {
                        tienGiamTuDiem = hoaDon.TongTien;
                        // Tính lại số điểm cần dùng để giảm đúng bằng tổng tiền
                        diemSuDung = (int)Math.Ceiling((hoaDon.TongTien / 10000m) * 100);
                    }
                    else
                    {
            ...
            decimal tongTienCuoi = hoaDon.TongTien - tienGiamTuDiem;
            if (tongTienCuoi < 0)
                tongTienCuoi = 0;
            ...
            if (maKH.HasValue)
            {
                ...
                if (khachHang != null)
                {
                    ...
                    decimal tongTienBanDau = hoaDon.TongTien + tienGiamTuDiem;
